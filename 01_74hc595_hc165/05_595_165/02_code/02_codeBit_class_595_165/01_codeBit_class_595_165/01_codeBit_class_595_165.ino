...
void setup() {
  ...
  Serial.print("Faker what what was that........");
  Serial.println("");
...
void write_n_595()
{
  for(int j=1;j<=3;j++)
  {
    ...
    {
      write_1_595(0x00);
      write_1_595(DrawerCtrl[i].getCtrl());
    }
   // delay(100);
  }
...
void read_n_165()
{
  ...
  digitalWrite(LATCH165,HIGH);
   // for(int j=1;j<=2;j++){
      for(int i=maxDrawer-1;i>=0;i--)
      {
        //read_1_165(DrawerCtrl[i].getLook());
        uint8_t a=read_1_165();
        DrawerCtrl[i].setLook(a);
        read_1_165();
        Serial.println(DrawerCtrl[i].getLook());
      }
       //delay(100);
   // }
// write_n_595();
...
void lock_ctrl(String cmd,int drawerId)
{
    if(cmd=="OpenDoor")
    {
      ...
      DrawerCtrl[drawerId].setOnCtrl(lock_bit);
    }else if(cmd=="CloseDoor")
    {
      //DrawerCtrl[drawerId].ctrl&=~(lock_bit);
       DrawerCtrl[drawerId].setOffCtrl(lock_bit);
    }
    write_n_595();
...
void light_ctrl(String cmd,int drawerId)
{
    if(cmd=="LightOn")
    {
        ...
        DrawerCtrl[drawerId].setOnCtrl(light_bit);
    }else if(cmd=="LightOff")
    {
       // DrawerCtrl[drawerId].ctrl&=~(light_bit);
        DrawerCtrl[drawerId].setOffCtrl(light_bit);
    }
        write_n_595();
}
//----------------------------------hàm xét trạng thái----------------------------------------
...
{
  read_n_165();
  return DrawerCtrl[drawerId].getLook() & lock_sts_bit? true:false; 
}
bool detect_light(int drawerId)
{
  read_n_165();
  return DrawerCtrl[drawerId].getLook() & light_sts_bit? true:false; 
}
//-----------------------------------main-----------------------------------------------------
void loop() {
